  ...
  fetch(allPokemonURL)
    ...
    .then((data) => {
      ...
      if (matchingPokemon.length === 0) {
        throw new Error(
          "No se encontró ningún Pokémon que coincida con la búsqueda."
        );
      }
      return Promise.all(
    ...
    .catch((error) => {
      ...
      alert(
        "No se encontró ningún Pokémon que coincida con la búsqueda. Inténtalo de nuevo."
      );
    });
}
    ...
    div.innerHTML = `
      ...
      <div class="pokemonTipos">
        ${poke.types
          .map(
            (type) =>
              `<p class="tipo" style="background-color: var(${
